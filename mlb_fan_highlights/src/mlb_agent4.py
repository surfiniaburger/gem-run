...
def generate_video_clips_node(state: AgentState) -> Dict[str, Any]:
    """Generates short video clips from selected static images using Veo."""
    node_start_time = time.time()
    logger.info("--- Generate Video Clips Node ---")
    generated_images = state.get("generated_visual_assets") or []
    current_videos = state.get("generated_video_assets") or [] # Get existing if looping (though unlikely here)
    ...
    if not genai_client:
        ...
        return {"generated_video_assets": current_videos} # Return existing

    if not generated_images:
        logger.info("No generated static images found to animate.")
        return {"generated_video_assets": current_videos}

    # --- Select images to animate ---
    images_to_animate = []
    # Simple selection: Prioritize images generated by Imagen/Veo, take up to MAX_IMAGES_TO_ANIMATE
    count = 0
    for asset in generated_images:
        if asset.get("type") == "generated_image" and asset.get("image_uri"):
             images_to_animate.append(asset)
             count += 1
             if count >= MAX_IMAGES_TO_ANIMATE:
                 break
    logger.info(f"Selected {len(images_to_animate)} generated images to animate (max: {MAX_IMAGES_TO_ANIMATE}).")

    if not images_to_animate:
        logger.info("No suitable generated images selected for animation.")
        return {"generated_video_assets": current_videos}
    ...
    for i, image_asset in enumerate(images_to_animate):
        image_gcs_uri = image_asset.get("image_uri")
        original_prompt = image_asset.get("prompt_origin", "unknown_prompt")
        logger.info(f"Attempting video generation {i+1}/{len(images_to_animate)} from image: {image_gcs_uri}")

        # --- Load image bytes ---
        load_result = load_image_bytes_from_gcs(image_gcs_uri)
        if not load_result:
            logger.error(f"Failed to load image bytes for {image_gcs_uri}. Skipping video generation.")
            time.sleep(VIDEO_GENERATION_ERROR_SLEEP_SECONDS) # Pause even on load failure
            continue
        image_bytes, mime_type = load_result

        # --- Prepare Veo API call ---
        try:
            # Create unique output URI for the video in the video bucket
            # Extract original image filename base
            base_filename = image_gcs_uri.split('/')[-1].split('.')[0]
            video_blob_name = f"videos/game_{game_pk}/vid_{base_filename}_{timestamp}.mp4"
            output_video_gcs_uri = f"gs://{GCS_BUCKET_GENERATED_VIDEOS}/{video_blob_name}"

            # Prepare image input for Veo
            veo_image_input = genai_types.Image(image_bytes=image_bytes)

            # Prepare config
            veo_config = genai_types.GenerateVideosConfig(
                output_gcs_uri=output_video_gcs_uri,
                aspect_ratio=VIDEO_GENERATION_ASPECT_RATIO, # Should match source image
                number_of_videos=1,
                person_generation=VIDEO_GENERATION_PERSON_ALLOW,
                # Potentially add safety settings if needed
            )

            # --- Call Veo API (starts long-running operation) ---
            logger.info(f"Starting Veo generation operation for {image_gcs_uri}...")
            operation = genai_client.models.generate_videos(
                model=VERTEX_VEO_MODEL_ID,
                image=veo_image_input,
                prompt=VIDEO_GENERATION_PROMPT, # Simple prompt for image-to-video
                config=veo_config,
            )
            logger.info(f"Veo operation started: {operation.operation.name}. Polling for completion...")
            ...
            while not operation.done:
                ...
                try:
                     ...
                     logger.debug(f"Polling Veo operation {operation.operation.name} (elapsed: {elapsed:.0f}s)... Done: {operation.done}")
                except Exception as poll_err:
                     logger.error(f"Error refreshing Veo operation {operation.operation.name}: {poll_err}. Stopping polling for this video.")
                     operation = None # Mark as failed
            ...
            if operation and operation.response:
                 ...
                 logger.info(f"Veo generation successful for {image_gcs_uri}. Video URI: {video_uri} (Polling took {polling_duration:.2f}s)")
                 newly_generated_videos.append({
                     "source_image_uri": image_gcs_uri,
                     "source_prompt": original_prompt,
                     "video_uri": video_uri,
                     "model_used": VERTEX_VEO_MODEL_ID,
                     "type": "generated_video"
                 })
            elif operation: # Operation finished but without a valid response/result
                 error_details = getattr(operation.operation.error, 'message', 'No error details provided.')
                 logger.error(f"Veo operation {operation.operation.name} finished WITH FAILED status for {image_gcs_uri}. Error: {error_details}. (Polling took {polling_duration:.2f}s)")
            else: # Operation polling was interrupted by an error
                logger.error(f"Veo operation for {image_gcs_uri} FAILED during polling.")


            # --- Sleep between starting next video generation ---
            ...
            logger.error(f"Veo Quota Exceeded starting generation for {image_gcs_uri}: {quota_error}. Sleeping {VIDEO_GENERATION_QUOTA_SLEEP_SECONDS}s.")
            time.sleep(VIDEO_GENERATION_QUOTA_SLEEP_SECONDS)
            # Skip to next image after quota error

        except Exception as e:
            logger.error(f"Unexpected error during Veo generation setup/start for {image_gcs_uri}: {e}", exc_info=True)
            ...
            # Skip to next image after other errors

    # --- Node Summary ---
    all_video_assets = current_videos + newly_generated_videos
    node_duration = time.time() - node_start_time
    logger.info(f"--- Video Clips Node Summary ---")
    logger.info(f"  Duration: {node_duration:.2f}s")
    logger.info(f"  Images attempted: {len(images_to_animate)}")
