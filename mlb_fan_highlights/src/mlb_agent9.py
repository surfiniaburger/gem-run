...
def assemble_video_node(state: AgentState) -> Dict[str, Any]:
    """
    Assembles the final video using MoviePy from generated assets.
    **Note:** This is a conceptual implementation. Needs robust error handling,
    sophisticated asset sequencing logic, and proper temp file management.
    """
    node_start_time = time.time()
    logger.info("--- Assemble Video Node ---")
    ...
    game_pk = state.get("game_pk", "unknown_game")
    #final_script = state.get("generated_content", "") # Get script for potential analysis
    ...
    if not word_timestamps:
         logger.warning("Word timestamps missing. Video timing will be approximate.")
         # Decide how to handle - e.g., equal duration for visuals?
    ...
    try:
        # 3. Download Essential Assets (Audio is primary)
        ...
        random.shuffle(available_visual_assets) # Simple random order for now

        # --- !!! This is where sophisticated sequencing logic is needed !!! ---
        # Example: Simple equal duration split (if no timestamps or basic approach)
        if not word_timestamps or True: # Using simple split for this example
            logger.info("Using simple equal duration split for visuals (Placeholder Logic).")
            num_visuals = len(available_visual_assets)
            if num_visuals == 0:
                raise ValueError("No visual assets available to assemble.")
            ...
            for i, asset in enumerate(available_visual_assets):
                ...
                if not asset_uri:
                    logger.warning(f"Skipping asset without URI: {asset}")
                    continue

                local_visual_path = download_gcs_blob(asset_uri, temp_dir)
                if not local_visual_path:
                    logger.warning(f"Failed to download visual asset: {asset_uri}. Skipping.")
                    continue
                local_files_to_clean.append(local_visual_path)

                clip = None
            try:
                if asset.get("type") == "generated_video" or asset_uri.lower().endswith((".mp4", ".mov", ".avi")):
                    # For videos, use subclip to truncate if necessary, instead of set_duration
                    temp_clip = VideoFileClip(local_visual_path)
                    # Ensure the calculated duration doesn't exceed the video's actual duration
                    actual_duration_to_use = min(temp_clip.duration, duration_per_visual)
                    if actual_duration_to_use <= 0:
                        logger.warning(f"Calculated duration for video {asset_uri} is zero or negative. Skipping.")
                        temp_clip.close() # Close the opened clip
                        
                    # Create the subclip
                    clip = temp_clip.subclip(0, actual_duration_to_use)
                    # It's good practice to close the original clip after creating the subclip,
                    # though MoviePy might handle this internally in some cases.
                    temp_clip.close()

                elif asset.get("type") in ["generated_image", "headshot", "logo"] or asset_uri.lower().endswith((".png", ".jpg", ".jpeg")):
                    # CORRECT WAY for ImageClip: Assign the result of set_duration back
                    if duration_per_visual <= 0:
                        logger.warning(f"Calculated duration for image {asset_uri} is zero or negative. Skipping.")
                        
                    # Create the ImageClip AND set its duration in one chain, assigning back to 'clip'
                    clip = ImageClip(local_visual_path).set_duration(duration_per_visual)

                else:
                    logger.warning(f"Unknown asset type or extension for {asset_uri}. Skipping.")
                    # Skip to the next asset

                # --- IMPORTANT: Check if clip was successfully created before proceeding ---
                if clip is None:
                    logger.error(f"Failed to create a valid MoviePy clip object for {asset_uri}. Skipping.")
                    

                # Set start time (removed as concatenate handles sequence)
                # current_time += actual_duration_to_use # Keep track if needed for other logic

                # Ensure clip resolution consistency (e.g., resize to 1280x720)
                target_width = 1280
                target_height = 720
                logger.debug(f"Resizing clip for {asset_uri} to {target_width}x{target_height}")
                clip = clip.resize(newsize=(target_width, target_height))

                visual_clips.append(clip)
                logger.debug(f"Prepared clip {i+1}/{num_visuals} for {asset_uri}, duration: {clip.duration:.2f}s")

            except Exception as clip_err:
                 # Log the specific error during clip processing
                 logger.error(f"Error processing visual asset {asset_uri} with MoviePy: {clip_err}", exc_info=True)
                 # Ensure any partially created clip object is closed to release resources
                 if 'temp_clip' in locals() and temp_clip:
                     try: temp_clip.close()
        ...
        # --- !!! End Placeholder Sequencing Logic !!! ---

        if not visual_clips:
            raise ValueError("No visual clips could be prepared for the final video.")

        # 5. Concatenate Visuals
        logger.info(f"Concatenating {len(visual_clips)} visual clips...")
        final_video_no_audio = concatenate_videoclips(visual_clips, method="compose") # Use 'compose' for potential overlaps/transparency if needed, else 'sequence'

        # 6. Set Audio
        logger.info("Setting final audio track...")
        final_video = final_video_no_audio.set_audio(main_audio_clip)

        # Ensure video duration matches audio duration *exactly* if needed
        # final_video = final_video.set_duration(total_audio_duration)

        # 7. Write Final Video File (Locally first)
        timestamp = datetime.now(UTC).strftime("%Y%m%d%H%M%S")
        ...
        # Use appropriate codecs, bitrate, threads, preset for quality/speed balance
        final_video.write_videofile(
            ...
            remove_temp=True,
            threads=4,             # Adjust based on your system cores
            fps=24,               # Standard film FPS, adjust if needed (e.g., 30)
            preset="medium",       # 'medium' is a balance. Use 'fast', 'slow', 'ultrafast' etc.
            logger='bar'           # Show progress bar
        )
        logger.info("Local video file written successfully.")
        local_files_to_clean.append(local_output_filename) # Add final video for potential cleanup

        # 8. Upload Final Video to GCS
        final_video_blob_name = f"{output_video_prefix}game_{game_pk}/final_recap_{timestamp}.mp4"
        final_video_gcs_uri = f"gs://{output_gcs_bucket}/{final_video_blob_name}"
        upload_blob(local_output_filename, final_video_gcs_uri)

    except Exception as e:
        logger.error(f"Error during video assembly: {e}", exc_info=True)
        return {"error": f"Failed during video assembly: {e}"}
    finally:
        # 9. Cleanup: Close clips and remove temp files/directory
        logger.info("Closing MoviePy clips...")
        if 'main_audio_clip' in locals() and main_audio_clip: main_audio_clip.close()
        if 'final_video_no_audio' in locals() and final_video_no_audio: final_video_no_audio.close()
        if 'final_video' in locals() and final_video: final_video.close()
        for clip in visual_clips: # Close individual visual clips
            try: clip.close()
...
workflow.add_edge("aggregate_final_output", "assemble_video") # Go to assembly after aggregation
workflow.add_edge("assemble_video", END) # End after assembly
