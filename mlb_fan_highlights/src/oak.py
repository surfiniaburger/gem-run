...
def process_team_data(team_name: str, team_id: int, n_games: int = 50, project_id: str = "your-project-id"):
    """Process recent games for a specific team and publish to Pub/Sub"""
    dataset_id = f"{team_name}_mlb_data_2024"
    topic_id = f"mlb-{team_name}-data-topic"
...
def main():
    """Process data for all teams and publish to Pub/Sub"""
    project_id = "gem-rush-007"  # Replace with your Google Cloud project ID

    for team_name, team_id in TEAMS.items():
        try:
            logger.info(f"Processing data for {team_name}...")
            process_team_data(team_name, team_id, project_id=project_id)
            logger.info(f"Successfully processed and published data for {team_name}.")
        except Exception as e:
            logger.error(f"Error processing data for {team_name}: {str(e)}")
            continue
