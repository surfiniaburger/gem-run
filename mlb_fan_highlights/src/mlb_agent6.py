...
# Instead of 'import moviepy.editor as mpy' use direct imports:
from moviepy import (
    VideoFileClip,
    AudioFileClip,
    ImageClip,
    ColorClip,
    TextClip,          # If you use TextClip
    CompositeVideoClip,
...
# from moviepy.audio.fx.all import ... # If you use audio effects like MultiplyVolume
...
# --- Configuration for Video Synthesis (Add near other configs) ---
VIDEO_TARGET_WIDTH = 1920
VIDEO_TARGET_HEIGHT = 1080
VIDEO_TARGET_FPS = 30
VIDEO_DEFAULT_IMAGE_DURATION_S = 5 # Default time to show each image if timing is approximate
VIDEO_IMAGE_FADE_S = 0.5 # Fade duration for images
VIDEO_KEN_BURNS_ZOOM_FACTOR = 1.15 # How much to zoom in for Ken Burns effect (e.g., 1.1 = 10% zoom)
GCS_BUCKET_FINAL_VIDEO = "mlb_final_videos" # Ensure this bucket exists!
...
# --- Helper Function: Download from GCS to Temporary File ---
def download_gcs_to_temp(gcs_uri: str, temp_dir: str) -> Optional[str]:
    """Downloads a GCS blob to a temporary file, returning the local path."""
    global storage_client
    if not gcs_uri or not gcs_uri.startswith("gs://") or not storage_client:
        logger.error(f"Invalid GCS URI or missing storage client: {gcs_uri}")
        return None
    try:
        bucket_name, blob_name = gcs_uri.replace("gs://", "").split("/", 1)
        bucket = storage_client.bucket(bucket_name)
        ...
        # Create a unique local filename in the temp directory
        ...
        logger.error(f"Error downloading {gcs_uri}: {e}", exc_info=True)
        return None

# --- Helper Function: Simple Ken Burns Effect ---
def apply_ken_burns(img_clip: ImageClip, duration: float, zoom_factor: float = 1.1) -> ImageClip:
    """Applies a simple zoom-in effect (Ken Burns) to an ImageClip."""
    try:
        target_w, target_h = img_clip.size

        def resize_func(t):
            # Calculate current scale: starts at zoom_factor, linearly scales down to 1.0
            scale = zoom_factor - (zoom_factor - 1.0) * (t / duration)
            return scale

        # Resize the clip dynamically over time
        resized_clip = img_clip.resize(resize_func)

        # Position the zoomed clip so it appears centered
        positioned_clip = resized_clip.set_position('center')

        # Ensure the final clip has the original target size by masking/compositing
        # Create a background or use CompositeVideoClip if needed, but often set_position handles it
        # For simplicity, let's assume positioning works well enough for now.
        # A more robust method might involve compositing onto a fixed-size canvas.
        return positioned_clip.set_duration(duration)

    except Exception as e:
        logger.error(f"Error applying Ken Burns effect: {e}", exc_info=True)
        return img_clip.set_duration(duration) # Return original clip on error

# --- NEW Video Synthesis Node ---
def synthesize_video_node(state: AgentState) -> Dict[str, Any]:
    """Combines audio, images, and video clips into a final video."""
    node_start_time = time.time()
    logger.info("--- Synthesize Final Video Node ---")

    # --- Get Required Assets from State ---
    ...
    final_script = state.get("generated_content", "") # For potential future use (subtitles)
    game_pk = state.get("game_pk", "unknown_game")

    # --- Basic Input Validation ---
    if not audio_uri:
        logger.error("Cannot synthesize video: Missing generated_audio_uri.")
        return {"error": "Missing audio URI for video synthesis."}
    if not all_image_assets and not all_video_assets:
        logger.warning("No image or video assets found to synthesize video.")
        # Decide: error or create audio-only? Let's return an error for now.
        return {"error": "Missing visual assets (images/videos) for video synthesis."}

    # --- Setup Temporary Directory ---
    temp_dir = tempfile.mkdtemp()
    logger.info(f"Created temporary directory for video synthesis: {temp_dir}")
    final_video_local_path = os.path.join(temp_dir, f"final_video_{game_pk}.mp4")
    final_video_gcs_uri = None
    all_clips_for_concat = []

    try:
        # --- 1. Download and Load Narration Audio ---
        logger.info("Loading narration audio...")
        local_audio_path = download_gcs_to_temp(audio_uri, temp_dir)
        if not local_audio_path:
            raise ValueError("Failed to download narration audio.")
        narration_audio = AudioFileClip(local_audio_path)
        total_audio_duration = narration_audio.duration
        logger.info(f"Narration audio loaded. Duration: {total_audio_duration:.2f} seconds.")

        # --- 2. Download and Process Video Clips ---
        logger.info(f"Processing {len(all_video_assets)} short video assets...")
        video_clips = []
        total_short_video_duration = 0
        for i, asset in enumerate(all_video_assets):
            video_gcs_uri = asset.get("video_uri")
            if not video_gcs_uri:
                logger.warning(f"Skipping video asset {i+1}: Missing 'video_uri'.")
                continue

            local_vid_path = download_gcs_to_temp(video_gcs_uri, temp_dir)
            if not local_vid_path:
                logger.warning(f"Skipping video asset {i+1}: Failed to download {video_gcs_uri}.")
                continue

            try:
                clip = VideoFileClip(local_vid_path, target_resolution=(VIDEO_TARGET_HEIGHT, VIDEO_TARGET_WIDTH))
                # Optional: Resize if needed, though Veo output might already be correct
                # clip = mpy_resize(clip, newsize=(VIDEO_TARGET_WIDTH, VIDEO_TARGET_HEIGHT))
                video_clips.append(clip)
                total_short_video_duration += clip.duration
                logger.info(f"  -> Loaded short video {i+1}: Duration {clip.duration:.2f}s")
            except Exception as e:
                logger.error(f"Error loading video clip {video_gcs_uri}: {e}. Skipping.")
        logger.info(f"Total duration of loaded short videos: {total_short_video_duration:.2f}s")


        # --- 3. Calculate Timing & Process Image Clips ---
        logger.info(f"Processing {len(all_image_assets)} image assets...")
        image_clips = []
        remaining_duration_for_images = total_audio_duration - total_short_video_duration

        if not all_image_assets:
             logger.info("No image assets to process.")
             image_duration_per = 0
        elif remaining_duration_for_images <= 0:
            logger.warning(f"Short video duration ({total_short_video_duration:.2f}s) exceeds or matches audio duration ({total_audio_duration:.2f}s). Images will have minimal/zero display time.")
            image_duration_per = 0.1 # Assign a tiny duration just to include them if needed
        else:
            image_duration_per = remaining_duration_for_images / len(all_image_assets)
            logger.info(f"Calculated duration per image: {image_duration_per:.2f}s")

        if image_duration_per < VIDEO_IMAGE_FADE_S * 2 and image_duration_per > 0:
             logger.warning(f"Image duration ({image_duration_per:.2f}s) is less than total fade time ({VIDEO_IMAGE_FADE_S*2:.2f}s). Fades might overlap or look abrupt.")
             actual_fade_time = max(0.1, image_duration_per / 3) # Reduce fade time drastically
        else:
             actual_fade_time = VIDEO_IMAGE_FADE_S


        for i, asset in enumerate(all_image_assets):
            img_gcs_uri = asset.get("image_uri")
            if not img_gcs_uri:
                logger.warning(f"Skipping image asset {i+1}: Missing 'image_uri'.")
                continue

            local_img_path = download_gcs_to_temp(img_gcs_uri, temp_dir)
            if not local_img_path:
                logger.warning(f"Skipping image asset {i+1}: Failed to download {img_gcs_uri}.")
                continue

            try:
                # Load image with Pillow to check size (optional but good practice)
                # pil_img = PilImage.open(local_img_path)
                # img_w, img_h = pil_img.size
                # pil_img.close() # Close file handle

                # Create ImageClip, resize to fit target height/width while maintaining aspect ratio
                img_clip_raw = ImageClip(local_img_path)

                # Resize logic (fit within target dimensions)
                # Use moviepy's resize which handles aspect ratio well
                img_clip_resized = mpy_resize(img_clip_raw, width=VIDEO_TARGET_WIDTH, height=VIDEO_TARGET_HEIGHT) # Fit within target

                # Create a black background clip and composite the image onto it (ensures fixed size)
                background = ColorClip(size=(VIDEO_TARGET_WIDTH, VIDEO_TARGET_HEIGHT), color=(0,0,0), ismask=False, duration=image_duration_per)
                img_on_bg = CompositeVideoClip([background, img_clip_resized.set_position('center')],
                                                  size=(VIDEO_TARGET_WIDTH, VIDEO_TARGET_HEIGHT))

                # Apply Ken Burns & Fade
                if image_duration_per > 0.5: # Only apply Ken Burns if duration is reasonable
                    img_on_bg = apply_ken_burns(img_on_bg, duration=image_duration_per, zoom_factor=VIDEO_KEN_BURNS_ZOOM_FACTOR)

                if actual_fade_time > 0 and image_duration_per > actual_fade_time * 2 : # Apply fade if duration allows
                    img_on_bg = fadein(img_on_bg, actual_fade_time)
                    img_on_bg = fadeout(img_on_bg, actual_fade_time)

                image_clips.append(img_on_bg.set_duration(image_duration_per))
                logger.info(f"  -> Processed image {i+1} ({img_gcs_uri}). Set duration: {image_duration_per:.2f}s")

            except Exception as e:
                logger.error(f"Error processing image clip {img_gcs_uri}: {e}. Skipping.")

        # --- 4. Assemble Final Video ---
        # Simple assembly order: Interleave images and videos? Or all images then all videos?
        # Let's try interleaving roughly based on count, ensuring total duration matches audio.
        logger.info("Assembling final video sequence...")
        num_images = len(image_clips)
        num_videos = len(video_clips)
        num_total_visuals = num_images + num_videos

        # Basic interleaving (can be improved significantly)
        img_idx, vid_idx = 0, 0
        while img_idx < num_images or vid_idx < num_videos:
            # Add a video first if available and roughly proportional
            if vid_idx < num_videos and (num_images == 0 or img_idx >= (vid_idx + 1) * num_images / num_videos):
                 all_clips_for_concat.append(video_clips[vid_idx])
                 vid_idx += 1
            elif img_idx < num_images:
                 all_clips_for_concat.append(image_clips[img_idx])
                 img_idx += 1
            # Safety break if logic fails
            elif vid_idx < num_videos:
                 all_clips_for_concat.append(video_clips[vid_idx])
                 vid_idx += 1
            else:
                 break # Should not happen

        if not all_clips_for_concat:
             raise ValueError("No video or image clips were successfully processed to concatenate.")

        logger.info(f"Concatenating {len(all_clips_for_concat)} visual clips...")
        final_video_track = concatenate_videoclips(all_clips_for_concat, method="compose") # Use compose for fades

        # Check duration mismatch
        visual_duration = final_video_track.duration
        logger.info(f"Total visual track duration BEFORE audio sync: {visual_duration:.2f}s")
        if abs(visual_duration - total_audio_duration) > 0.5: # Allow minor discrepancy
            logger.warning(f"Visual track duration ({visual_duration:.2f}s) differs significantly from audio duration ({total_audio_duration:.2f}s). Adjusting video duration to match audio.")
            # Force video duration to match audio. This might cut off the end or add black frames.
            final_video_track = final_video_track.set_duration(total_audio_duration)

        # Set the audio
        logger.info("Attaching narration audio to video track...")
        final_clip = final_video_track.set_audio(narration_audio)

        # --- 5. Write Video File ---
        logger.info(f"Writing final video file to {final_video_local_path}...")
        final_clip.write_videofile(
            final_video_local_path,
            codec='libx264',        # Common, good quality codec
            audio_codec='aac',      # Common audio codec
            temp_audiofile=os.path.join(temp_dir, 'temp_audio.m4a'), # Explicit temp audio file
            remove_temp=True,
            preset='medium',        # Balance between speed and quality/size ('slow' is better quality but slower)
            fps=VIDEO_TARGET_FPS,
            threads=4,              # Use multiple threads if available
            logger='bar'            # Show progress bar
        )
        logger.info("Final video file written successfully.")

        # --- 6. Upload to GCS ---
        timestamp = datetime.now(UTC).strftime("%Y%m%d%H%M%S")
        video_blob_name = f"final_videos/game_{game_pk}/mlb_recap_{timestamp}.mp4"
        bucket = storage_client.bucket(GCS_BUCKET_FINAL_VIDEO)
        blob = bucket.blob(video_blob_name)

        logger.info(f"Uploading final video to gs://{GCS_BUCKET_FINAL_VIDEO}/{video_blob_name}...")
        blob.upload_from_filename(final_video_local_path)
        final_video_gcs_uri = f"gs://{GCS_BUCKET_FINAL_VIDEO}/{video_blob_name}"
        logger.info("Upload complete.")

    except Exception as e:
        logger.error(f"Error during video synthesis: {e}", exc_info=True)
        # Preserve existing error, add new one if needed
        error_msg = f"Failed during video synthesis: {e}"
        existing_error = state.get("error")
        final_error = f"{existing_error}; {error_msg}" if existing_error else error_msg
        return {"error": final_error, "final_video_uri": None} # Return error state
    finally:
        # --- Cleanup ---
        logger.info(f"Cleaning up temporary directory: {temp_dir}")
        # Close any open moviepy clips explicitly if necessary (usually handled by context managers or GC)
        # Make sure narration_audio, video_clips[*], image_clips[*] are closed if they hold file handles
        # Example (might not be needed if write_videofile closes sources):
        # if 'narration_audio' in locals() and narration_audio: narration_audio.close()
        # for clip in video_clips: clip.close()
        # for clip in image_clips: clip.close() # ImageClips might not need closing
        # if 'final_video_track' in locals() and final_video_track: final_video_track.close()
    ...
    node_duration = time.time() - node_start_time
    logger.info(f"--- Final Video Synthesis Node Summary ---")
    logger.info(f"  Duration: {node_duration:.2f}s")
    logger.info(f"  Final Video URI: {final_video_gcs_uri}")

    # Return the final state with the video URI
    output_state = state.copy()
    output_state["final_video_uri"] = final_video_gcs_uri
    # Clear error if we succeeded here, otherwise it's set in the except block
    if final_video_gcs_uri:
       output_state["error"] = None # Clear any previous non-fatal errors if synthesis succeeded
...
workflow.add_node("synthesize_video", synthesize_video_node)             # NEW: Creates final video
...
# Audio generation node leads to the NEW video synthesis node
workflow.add_edge("generate_audio", "synthesize_video")

# NEW: Final edge from audio generation to END
workflow.add_edge("synthesize_video", END)
    ...
    try:
        ...
        if final_state.get("error"):
            ...
            if final_state.get("generated_audio_uri"):
                ...
                print("\n** FINAL Synthesized Video URI: **") # Display the final output
                print(final_state.get("final_video_uri", "N/A - Synthesis might have failed, check logs."))
        else:
