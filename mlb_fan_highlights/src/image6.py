...
def display_images_in_grid(images):
    ...
    ncols = min(len(images), 4)  # Adjust columns based on the number of images

    # Handle the case with a single image
    if len(images) == 1:
        plt.figure(figsize=(6, 6))
        plt.imshow(images[0].image._pil_image)
        plt.axis('off')
        plt.show()
        return
    ...
    # Handle the case when there's only one row but multiple columns
...
def save_images(images, prefix):
    ...
    # Check if images is None or empty
    if not images:
        print(f"No images to save for {prefix}")
        return
...
def generate_content(prompt):
    ...
    try:
        ...
        # Return a simple fallback string that won't cause JSON parsing errors
        return "{}"
...
def enhance_image_prompt(basic_prompt):
    ...
    Keep the prompt under 100 words total.
    """
    
    text_prompt = prompt_template.format(image_prompt=basic_prompt)
    enhanced_prompt = generate_content(text_prompt)
    
    # If the enhanced prompt is longer than 250 characters, use the original prompt
    # This helps avoid issues with overly complex prompts that might cause generation failures
    if len(enhanced_prompt) > 250:
        print("Enhanced prompt too long, reverting to basic prompt")
        return basic_prompt
...
def generate_storyboard_prompts(game_data):
    ...
    """
    prompt = f"""
    Create a list of 5 visual storyboard prompts for an MLB podcast about this game:
    ...
    Keep each "basic_prompt" short and clear, under 50 characters if possible.
    Output only the properly formatted JSON, nothing else.
    """
...
def generate_images_safely(model, prompt, num_images):
    ...
    try:
        ...
        # Return an empty list if image generation fails
        return []


def generate_game_visuals(transcript, num_images_per_prompt=2):
    """
    ...
    for prompt_data in storyboard_prompts:
        ...
        # Generate images with error handling
        generated_images = generate_images_safely(
            IMAGEN_MODEL,
            enhanced_prompt,
